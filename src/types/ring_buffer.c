...
//
word rb_word_full(p_word_ring r) {
...
//
word rb_word_empty(p_word_ring r) {
...
//
void rb_word_put(p_word_ring r, word data) {
    if (!rb_word_full(r)) {
...
//
word rb_word_get(p_word_ring r) {
    if (!rb_word_empty(r)) {
        word data = r->buffer[r->tail++];
