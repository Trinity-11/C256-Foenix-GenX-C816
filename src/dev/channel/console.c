...
//
short con_write_b(p_channel chan, byte b) {
    txt_putc(chan->dev, (char)b);
}
...
//
short con_read(p_channel chan, byte * buffer, short size) {
    int i;
...
//
short con_readline(p_channel chan, byte * buffer, short size) {
    int i = 0;
...
//
short con_write(p_channel chan, byte * buffer, short size) {
    int i;
...
short con_ioctrl(p_channel chan, short command, byte * buffer, short size) {
    return 0;
}
